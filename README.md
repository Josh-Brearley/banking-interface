...
- **Lightweight design system** in [`src/components/ui`](./src/components/ui) (Button, Input,
  Select, Card, Table, Modal, Drawer, Avatar, Badge, Skeleton, EmptyState, ErrorState) driven
  by design tokens and `class-variance-authority`, brand-aligned to the Eagle Bank logo
  (navy + cyan) with self-hosted Inter.
- **99 tests** across 23 files (Vitest + React Testing Library), each tracing to a spec
...
├── components/
│   ├── ui/       # design-system primitives (zero feature knowledge)
│   └── shared/   # composed, feature-agnostic widgets (DataTable, StatCard, MoneyAmount…)
...
features never import each other's internals; `components/ui` knows nothing about features or
services; `services` are pure async functions with no React. Cross-feature communication happens
...
- **99 tests** across 23 files (Vitest + React Testing Library + `user-event`), with a custom
...
- **CI/CD** — GitHub Actions running `lint · typecheck · test (coverage) · build` on every PR,
  plus preview deployments and a Lighthouse CI budget gate.
- **E2E tests** — Playwright for the critical journeys (login → dashboard → transfer), and visual
...
- **Design-system package extraction** — promote `components/ui` + tokens into a versioned package
  (with Storybook and visual tests) shared across products/teams.
...
src/components ui (design system) · shared (DataTable, StatCard, MoneyAmount, Pagination…)
